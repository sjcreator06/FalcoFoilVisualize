...
def foilData():
    ...
    global mediumFoilIndexSorted,thickFoilIndexSorted
...
def GUIFoilTransform(foilCategory,firstFoil,secondFoil):
    foilData()
    if foilCategory.lower() == "medium":
        # Plotting Reference Plots
        plt.plot(x3[firstFoil],y3[firstFoil])
...
def foilTransformFigure():
    # Airfoil Tranformation Animation Figure
    global figure,foil
...
if __name__ == "__main__":
    # Main Query to Run Animation
    mainQuery = input("Would you like to run the foil transformation animation without GUI? (yes/no): ")

    if mainQuery.lower() == "yes":
        foilData()
        foilTransformFigure()
        userFoilInput()
    ...
    elif mainQuery.lower() == "no":
        print("Use GUI to run the foil transformation animation!")
